...
8. produce internal, review and public/LewesLive outputs separately.
...
| JSON-LD | Organisation-owned structured data from configured websites | None |
| Manual CSV | Curated corrections, existing LewesLive listings and hand-verified records | None |
...
- Postcodes.io (planned enrichment adapter): `https://postcodes.io/docs/overview/`
- CQC data access (planned regulated-care adapter): `https://www.cqc.org.uk/about-us/transparency/using-cqc-data`
- Charity Commission API (planned charity adapter): `https://register-of-charities.charitycommission.gov.uk/en/documentation-on-the-api`
...
public/
  ...
  manifest.v1.json
leweslive/
  directory.v1.json           LewesLive ingestion contract
...
- it comes from an authoritative Class A register appropriate to that entity type (for example FHRS food establishments);
...
This matters particularly for sole traders, home businesses and Companies House registered offices.
...
The architecture intentionally leaves room for category-aware verification adapters including CQC, Charity Commission, Gas Safe/NICEIC claims where lawful/available, local-authority data, Chamber/member directories and controlled website discovery. New sources should create candidates and provenance; they should not bypass the publication gate.
